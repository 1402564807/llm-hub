...
export function useUser(tableRef: Ref) {
  ...
  const columns: TableColumnList = [
    ...
    {
      label: "余额",
      cellRenderer: ({ row }) => <span>{row.allQuota - row.usedQuota}</span>,
      minWidth: 130
    },
    {
      label: "邮箱",
      ...
      minWidth: 90,
      formatter: ({ email }) =>
        hideTextAtIndex(email, { start: 3, end: email.length })
    },
    {
      label: "状态",
      prop: "enable",
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate((valid: boolean) => {
          if (valid) {
            ...
            if (title === "新增") {
              // 实际开发先调用新增接口，再进行下面操作
              addUser(curData).then(({ success }) => {
                if (success) {
                  chores();
                  return;
                }
                message(`${title}用户${curData.username}失败`, {
                  type: "error"
                });
              });
            } else {
              // 实际开发先调用修改接口，再进行下面操作
              updateUser(curData).then(({ success }) => {
                if (success) {
                  chores();
                  return;
                }
